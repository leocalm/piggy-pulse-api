...
```bash
cd /opt/piggypulse/piggy-pulse/deploy/production
cp .env.example .env
```

Set all secret values in `.env`, including:
- `PIGGY_PULSE_API_IMAGE` (GHCR digest reference)
- `PIGGY_PULSE_CRON_IMAGE` (GHCR digest reference)
- `PIGGY_PULSE_IMAGE_PLATFORM` (set to `linux/amd64` for Hetzner x86 servers)
...
docker compose ps
docker compose logs --tail=200 piggy-pulse
```
