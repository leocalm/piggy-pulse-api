...
export DATABASE_URL=postgres://user:password@localhost:5432/piggy_pulse_db
...
# Set only the piggy_pulse crate to debug
export RUST_LOG=piggy_pulse=debug

# Set specific modules to different levels
export RUST_LOG=piggy_pulse::routes=trace,piggy_pulse::database=debug,info

# Global info, but routes module at debug level
export RUST_LOG=info,piggy_pulse::routes=debug

# Trace specific route handlers
export RUST_LOG=piggy_pulse::routes::transaction=trace
```
...
```toml
[database]
url = "postgres://postgres:example@localhost:5432/piggy_pulse_db"
max_connections = 4
...
```bash
export PIGGY_PULSE_DATABASE_URL=postgres://prod-user:secure-pass@prod-host:5432/piggy_pulse_db
...
export DATABASE_URL=postgres://prod-db/piggy_pulse_db
cargo run
```
...
PIGGY_PULSE_DATABASE_URL=postgres://localhost/piggy_pulse_test cargo test
```
...
Example:
```
postgres://postgres:example@localhost:5432/piggy_pulse_db
```
