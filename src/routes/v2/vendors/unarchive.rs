...
#[post("/<id>/unarchive")]
pub async fn unarchive_vendor(pool: &State<PgPool>, user: CurrentUser, id: &str) -> Result<Json<VendorResponse>, AppError> {
    let uuid = Uuid::parse_str(id).map_err(|e| AppError::uuid("Invalid vendor id", e))?;
    let repo = PostgresRepository { pool: pool.inner().clone() };
    let service = VendorService::new(&repo);

    let response = service.unarchive_vendor(&uuid, &user.id).await?;
    Ok(Json(response))
}
