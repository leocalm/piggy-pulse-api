...
pub async fn exclude_overlay_transaction(pool: &State<PgPool>, user: CurrentUser, id: &str, tx_id: &str) -> Result<Status, AppError> {
    ...
    service.exclude_transaction(&overlay_uuid, &tx_uuid, &user.id).await?;
    Ok(Status::NoContent)
}
