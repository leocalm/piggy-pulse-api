...
use crate::dto::dashboard::CurrentPeriodHistoryResponse;
use crate::error::app_error::AppError;
use crate::service::dashboard::DashboardService;

#[get("/current-period-history?<periodId>")]
pub async fn get_current_period_history(
    pool: &State<PgPool>,
    user: CurrentUser,
    #[allow(non_snake_case)] periodId: Option<String>,
) -> Result<Json<CurrentPeriodHistoryResponse>, AppError> {
    ...
    let response = service.get_current_period_history(&period_uuid, &user.id).await?;
    Ok(Json(response))
}
