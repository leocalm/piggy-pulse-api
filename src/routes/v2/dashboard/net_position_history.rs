...
use crate::dto::dashboard::NetPositionHistoryResponse;
use crate::error::app_error::AppError;
use crate::service::dashboard::DashboardService;

#[get("/net-position-history?<periodId>")]
pub async fn get_net_position_history(
    pool: &State<PgPool>,
    user: CurrentUser,
    #[allow(non_snake_case)] periodId: Option<String>,
) -> Result<Json<NetPositionHistoryResponse>, AppError> {
    ...
    let response = service.get_net_position_history(&period_uuid, &user.id).await?;
    Ok(Json(response))
}
