...
use crate::dto::dashboard::VariableCategoriesResponse;
use crate::error::app_error::AppError;
use crate::service::dashboard::DashboardService;

#[get("/variable-categories?<periodId>")]
pub async fn get_variable_categories(
    pool: &State<PgPool>,
    user: CurrentUser,
    #[allow(non_snake_case)] periodId: Option<String>,
) -> Result<Json<VariableCategoriesResponse>, AppError> {
    ...
    let response = service.get_variable_categories(&period_uuid, &user.id).await?;
    Ok(Json(response))
}
