...
"#,
    ...
    /// Generate HTML version of emergency 2FA disable email
    fn generate_emergency_2fa_disable_html(&self, to_name: &str, disable_link: &str) -> String {
        format!(
            r##"
<!DOCTYPE html>
<html lang="en" xmlns="http://www.w3.org/1999/xhtml" xmlns:v="urn:schemas-microsoft-com:vml" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word">
<head>
    ...
    <meta name="color-scheme" content="light">
    <meta name="supported-color-schemes" content="light">
    <title>Emergency Two-Factor Authentication Disable</title>
    <!--[if mso]>
    <xml>
        <o:OfficeDocumentSettings>
            <o:PixelsPerInch>96</o:PixelsPerInch>
    ...
    <style>
        ...
        .button {{
            ...
            background-image: linear-gradient(135deg, #FA5252 0%, #E03131 100%);
            color: #FFFFFF !important;
            text-decoration: none;
            font-size: 15px;
            font-weight: 700;
...
<body id="body">
    <div class="preheader">Emergency request to disable two-factor authentication. This secure link expires in 15 minutes.</div>
    <table role="presentation" class="wrapper" width="100%" cellspacing="0" cellpadding="0" border="0">
      <tr>
        <td align="center">
          <table role="presentation" class="card" width="640" cellspacing="0" cellpadding="0" border="0">
            ...
            <tr>
              <td class="content">
                <p class="eyebrow">Security Alert</p>
                <h1 class="title">Emergency 2FA Disable Request</h1>
                <p class="body-text">Hi {},</p>
                <p class="body-text">We received an emergency request to disable two-factor authentication on your PiggyPulse account. If you made this request, use the button below to confirm.</p>

                <div class="cta-wrap">
                  <!--[if mso]>
                  <v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" href="{}" style="height:44px;v-text-anchor:middle;width:240px;" arcsize="18%" stroke="f" fillcolor="#FA5252">
                    <v:fill type="gradient" color="#FA5252" color2="#E03131" angle="315"/>
                    <w:anchorlock/>
                    <center style="color:#FFFFFF;font-family:Segoe UI, Arial, sans-serif;font-size:15px;font-weight:700;">
                  ...
                  <![endif]-->
                  <!--[if !mso]><!-- -->
                  <a href="{}" class="button">Disable Two-Factor Auth</a>
                  <!--<![endif]-->
                </div>
                ...
                <div class="warning">
                  <p class="warning-title">⚠️ Critical Security Warning</p>
                  <ul class="warning-list">
                    <li>Disabling 2FA will make your account significantly less secure.</li>
                    <li>Only proceed if you have lost access to your authenticator device.</li>
                    <li>You can re-enable 2FA anytime after regaining account access.</li>
                    <li>If you did NOT request this, someone may be trying to compromise your account.</li>
                  </ul>
                </div>
            ...
            <tr>
              <td class="footer">
                <p class="footer-text">If you did NOT request this emergency disable, please ignore this email and ensure your account password is secure. Consider changing your password if you suspect unauthorized access.</p>
                <p class="footer-signoff">PiggyPulse Security Team</p>
              </td>
            </tr>
          </table>
        </td>
...
"##,
            to_name, disable_link, disable_link, disable_link
        )
    }

    /// Generate plain text version of emergency 2FA disable email
    fn generate_emergency_2fa_disable_text(&self, to_name: &str, disable_link: &str) -> String {
        format!(
            r#"PiggyPulse | Emergency Two-Factor Authentication Disable

Hi {},

We received an emergency request to disable two-factor authentication on your PiggyPulse account.

If you made this request, use the secure link below to confirm:
{}
...
If you did NOT request this emergency disable, please ignore this email and ensure your account password is secure. Consider changing your password if you suspect unauthorized access.

PiggyPulse Security Team
"#,
            to_name, disable_link
        )
    }
...
mod tests {
    ...
    fn test_generate_emergency_2fa_disable_html() {
        ...
        let service = EmailService::new(config);
        let html = service.generate_emergency_2fa_disable_html("John Doe", "https://example.com/2fa/emergency?token=abc123");

        assert!(html.contains("John Doe"));
        assert!(html.contains("https://example.com/2fa/emergency?token=abc123"));
        assert!(html.contains("Disable Two-Factor Auth"));
        assert!(html.contains("15 minutes"));
        assert!(html.contains("Critical Security Warning"));
        assert!(html.contains("significantly less secure"));
    }
    ...
    fn test_generate_emergency_2fa_disable_text() {
        ...
        let service = EmailService::new(config);
        let text = service.generate_emergency_2fa_disable_text("Jane Smith", "https://example.com/2fa/emergency?token=xyz789");

        assert!(text.contains("Jane Smith"));
        assert!(text.contains("https://example.com/2fa/emergency?token=xyz789"));
        assert!(text.contains("CRITICAL SECURITY WARNING"));
        assert!(text.contains("15 minutes"));
        assert!(text.contains("significantly less secure"));
    }
}
