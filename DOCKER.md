...
```bash
DATABASE_URL=postgres://postgres:<password>@<host>:5432/piggy-pulse_db \
...
- **Database**: piggy-pulse_db (or your POSTGRES_DB)
...
docker compose exec db psql -U postgres -d piggy-pulse_db -c "SELECT 1;"
```
...
```bash
docker compose exec db pg_dump -U postgres piggy-pulse_db > backup.sql
```

### Restore Database

```bash
cat backup.sql | docker compose exec -T db psql -U postgres piggy-pulse_db
```
